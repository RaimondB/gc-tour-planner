...
- **Promotion model (dev → UAT → PR → main → prod).** **dev** is the author/agent inner loop. **UAT** (the `gctp` stack on the shared host) runs the **in-flight feature branch** for the owner's acceptance before the PR — check it out and `docker compose up --build -d` (a web-only change can rebuild just `web`). The **PR + green CI is the final validation gate**. **prod tracks `main` on a separate host** and is promoted **manually** after merge (`git checkout main && git pull --ff-only && docker compose up --build -d`). Keep prod's checkout exactly on `main`. Full detail: [docs/sdlc/release-and-deploy.md](docs/sdlc/release-and-deploy.md), [branching-and-prs.md](docs/sdlc/branching-and-prs.md).
