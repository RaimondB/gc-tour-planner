...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.warn(`(A = current, B = boundary-spanning grow; ${seedsCompared} seeds)`);
    console.warn(`foreign ids in subgraphs   : A ${foreignA}  →  B ${foreignB}`);
    console.warn(`returned candidates (total): A ${candCountA}  →  B ${candCountB}`);
    console.warn(`caches across candidates    : A ${cacheCoverA}  →  B ${cacheCoverB}`);
    console.warn(`mean candidate similarity   : ${(mean(simScores) * 100).toFixed(1)}% (Jaccard A→B; 100% = unchanged)`);
    console.warn(`seeds with identical results: ${seedsIdentical}/${seedsCompared}`);
    console.warn(`seeds w/ changed candidate # : ${seedsCandCountChanged}/${seedsCompared}`);
    console.warn("(foreignB≈0 ⇒ leak closed; higher caches-covered ⇒ clusters recovered across the boundary)");
  } finally {
    await app.close();
  }
}
