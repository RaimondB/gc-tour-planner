...
- `getLanduseInBBox(bbox, kinds)` first reads `osm_landuse` by `area_hash`. If absent or `fetched_at > 30 days ago`, enqueue `overpass-refresh`; serve stale data meanwhile (acceptable for soft preferences).
- Overpass query template:
  ```overpass
  [out:json][timeout:60];
  (
    way["landuse"~"^(forest|park|residential|farmland|industrial|meadow|...)$"]({{bbox}});
    relation["landuse"~"..."]({{bbox}});
    way["natural"~"^(wood|water|wetland|heath|scrub)$"]({{bbox}});
  );
  out geom;
  ```
- Use the public Overpass endpoint by default; allow override via env (`OVERPASS_URL`) for self-hosted Overpass.
- Valkey lock: `SET lock:overpass:<bbox-hash> 1 NX EX 60` — first caller fetches, others wait + read the resulting cache row.
