...
- **Run a separate Overpass for the dev compose project.** Rejected — same reasoning as OSRM in [CLAUDE.md](../../CLAUDE.md): the second instance OOMs the host and duplicates a 30-min cold start for no benefit. Dev's Overpass calls are read-only HTTP just like OSRM, no data risk to UAT.
