...
| `PLANNER_CLUSTER_GROW` | _unset_ | Boundary-spanning clusters ([ADR-0024](adr/0024-boundary-spanning-clusters.md)). Set `1`/`true` to let a cluster that straddles the search circle grow past it: discovery fetches caches out to `radiusM + distanceBudgetMeters/2`, seeds only from the in-radius subset, and constrains the graph to the pool (closing the `refine→pool invariant broken` leak). Default-off = legacy hard cutoff at `radiusM`. Growth is bounded by the existing `maxCaches`/`distanceBudgetMeters` caps and `MAX_DISCOVERY_POOL`. |
