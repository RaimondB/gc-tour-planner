...
| `PLANNER_CENTER_BIAS_WEIGHT` | `1` | Default weight on the `centerProximity` score term — a linear falloff of a cluster centroid's distance from the search `center` (`1` at the centre, `0` at the radius edge, clamped to `0` beyond). Pushes clusters near the centre of the search area up the ranking, countering the "clusters surface at the edge" artifact (ranking-only; it does **not** change which clusters are found or reduce coverage). Per-request override: `softPreferences.centerProximityWeight` (the web "Prefer central clusters" slider, `0..5`). `0` disables the bias. With `PLANNER_CLUSTER_GROW=1`, boundary-straddling centroids past `radiusM` simply lose the bonus (no penalty), so a high bias partially counteracts grow's intent. |
