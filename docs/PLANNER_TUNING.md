...
| -------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PLANNER_CLUSTERING` | `louvain` | Strategy used to partition the walking graph. Alternatives: `dbscan`, `hdbscan`, `hdbscan-star`, `components`. Louvain has produced the most visibly-sensible clusters in field testing. `hdbscan-star` is true HDBSCAN\* (condensed-tree stability extraction) and is the density-aware option to A/B against the legacy `hdbscan` (recursive MST bisection, which tends to over-split loose-but-real pods). |
...
| `PLANNER_KNN_SYMMETRY` | `or` | How a k-NN edge becomes undirected. `or` (default, legacy): keep the edge if **either** endpoint ranks the other in its top-k. `mutual` ("dual-link"): keep it only if **both** do, with a min-degree floor that re-adds an otherwise-orphaned node's single nearest edge. Mutual kills one-way "hub" links that fuse distinct pods → sharper cluster separation, at some recall cost in sparse/rural areas (the floor mitigates orphaning). Benefits every clustering strategy; A/B via the explain endpoint. |
