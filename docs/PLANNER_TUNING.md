...
| -------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PLANNER_CLUSTERING` | `louvain` | Strategy used to partition the walking graph. Alternatives: `leiden`, `dbscan`, `hdbscan`, `hdbscan-star`, `components`. Louvain has produced the most visibly-sensible clusters in field testing. `leiden` adds Louvain's missing refinement phase so every returned community is internally connected (hand-rolled, no extra dep). `hdbscan-star` is true HDBSCAN\* (condensed-tree stability extraction) and is the density-aware option to A/B against the legacy `hdbscan` (recursive MST bisection, which tends to over-split loose-but-real pods). |
