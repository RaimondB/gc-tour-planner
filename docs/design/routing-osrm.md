...
- **OSRM internal parallelism.** `osrm-routed --threads N` is set from `OSRM_THREADS` (default 4) in [infra/osrm/bootstrap.sh](../../infra/osrm/bootstrap.sh). NUC is 4C/8T; default keeps headroom for postgres + api. Raise for higher single-tenant throughput; lower if OSRM starves the rest of the stack.
