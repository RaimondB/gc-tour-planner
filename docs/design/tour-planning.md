...
#### Future work — metric-closure on the rep matrix

The connected-component pre-filter guarantees the candidate set is **connected**, not that every *direct* rep pair is routable: two reps in the same `≤ maxLinkMeters` component can still have a `null` direct leg (no OSRM foot route between them, only via intermediates). A tour that happens to place such a pair consecutively then carries a `null` leg → the trim's loop length reads `∞` and it can mildly **over-trim**. (Plans still *return* — both planners sanitise non-finite numbers at the wire boundary so `Infinity`/`NaN` never serialises to JSON `null` and breaks the web's `PlanResult.parse`.) The proper cure is to compute the **metric closure** of the kept component (all-pairs shortest path over the `≤ maxLinkMeters` graph) and fill `null` direct legs with that distance, so the solver/trim never see `∞` and always work on a fully-finite matrix. Pair it with a server-side `PlanResult.parse` guard before returning, to catch any future non-finite leak loudly rather than silently. Not yet implemented.
