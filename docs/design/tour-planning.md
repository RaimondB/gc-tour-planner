...
2. Project each cache to **local equirectangular meters** around `center` (cheap; accurate over our radius).
   - **Adventure-Lab collapse (FR-I17, [ADR-0034](../adr/0034-adventure-lab-collapse.md)):** before the pool cap and the walking graph, every Adventure Lab's stages collapse to **one representative node** (the stage nearest the adventure's centroid — a real, routable cache), so an adventure counts as a single place instead of a 5–10-stage blob that would form its own micro-cluster. The chosen cluster's ids are expanded back to all stages when candidates are scored/returned, so Pass-2 still routes the whole adventure. No-op when no AL is in the pool. When the user clears `includeAdventuresInClustering` ([FR-T15](../requirements/tour-planning.md)), AL stages are dropped from the candidate pool entirely before this step, so clusters form from ordinary caches only.
...
4. **Pre-trim** — marginal-cost trim over the OSRM-primary matrix, run before any leg picking. **Budget-aware by default** (`PLANNER_MARGINAL_BUDGET_AWARE`, default on): route the full cluster, then keep every cache while the routed loop (inter-cache legs + parking closure) fits `distanceBudgetMeters`, dropping only genuine outliers whose marginal `(leg_in + leg_out − skip)` exceeds the outlier floor `PLANNER_MARGINAL_OUTLIER_FACTOR × maxLinkMeters` (default 2×). When the loop exceeds the budget, iteratively drop the worst-marginal cache and re-2-opt until it fits. This stops the trim from cutting good caches before the budget is full — the symptom that left loops ~2 km short of budget. Setting `PLANNER_MARGINAL_BUDGET_AWARE=false` restores the legacy fixed-threshold trim (drop anything whose marginal exceeds `input.maxLinkMeters`, route-blind). The threshold migrated from the env-derived `resolveMarginalTrimThreshold` formula to the per-plan `maxLinkMeters` knob so both Pass-1 and Pass-2 trim respect the same user tolerance.
...
9. Compose `PlanResult` with score breakdown + `droppedCacheIds` (union of pre-trim + post-trim drops; the UI renders these as gray-x markers) **plus `droppedCaches`** — the same ids with a per-cache **reason** so the UI answers "why was this removed?" on click ([FR-T13](../requirements/tour-planning.md)). Reasons: `budget` / `outlier` (marginal trim, carry `neededBudgetMeters` = the cache's marginal so the UI can suggest "raise your budget by ~X"), `fringe` (post-leg-pick overlap trim), `unreachable` (both planners now **pre-filter the candidate set to its largest walk-connected component** — `filterConnected` (greedy) and `largestConnectedComponent` (solver), edges capped at `maxLinkMeters` — and surface the dropped minority instead of dragging a `null`/cross-barrier leg into the loop; the solver also classifies a rep with sentinel parking legs / no finite matrix neighbour the same way), `adventure-incomplete` (solver AL-aware orphan trim), and `candidate-cap` (a whole adventure skipped before planning because `expandForCompleteAdventures` hit `AUGMENT_MAX_CACHES`, threaded onto the result in `ToursService.planLoop`). On the **solver path** every candidate the solver didn't visit is accounted for (not just the local trims), so AL-augmented routes never leave a cache silently absent. `droppedCacheIds === droppedCaches.map(d => d.id)` (lock-step); both default `[]` so pre-feature `StoredPlan` JSONB still parses.
...
Lives behind the same `TourPlannerStrategy` interface (see [ADR-0002](../adr/0002-planner-strategy-interface.md)). It is now **live for the Adventure-Lab path**: `ToursService.planLoop` selects greedy vs. solver per request (`TOUR_PLANNER=auto`), routing to the Timefold sidecar ([ADR-0005](../adr/0005-timefold-solver-sidecar.md)) when the candidate set contains an AL stage. The solver models what greedy can't: **atomic adventures** (HARD), **stage contiguity** (HARD, when interleave is off), **stage ordering** (HARD, for *linear* adventures — their stages appear in ascending `stageSequence` order while other caches may still interleave; data-driven per adventure from `adventure_sequential`, [FR-I18](../requirements/ingest.md)), and **loop shape** (a SOFT per-metre loop-length penalty on a three-level `HardMediumSoftLong` score, kept below the visited-count MEDIUM reward so count never loses to length). Nest pulls in an adventure's missing stages first, **filters the candidates to one walk-connected component** (so the solver never gets a set with an unrouteable `null` leg — which would collapse `Tour.totalMeters()` to ∞ → 0 and trigger a runaway over-trim), collapses co-located stages to one rep, runs the solve, then reuses the greedy toolkit (AL-aware marginal trim + loop-aware leg picker + co-location expansion) for the final geometry. After the trim a **budget-fill re-add** pass restores whole adventures (and non-AL caches) dropped for *budget* reasons while the loop stays within `distanceBudgetMeters` — so dropping one over-budget adventure lets the cheaper ones back in rather than leaving the loop far under budget (`outlier`/`unreachable` drops are never reclaimed). Full behaviour: [requirements FR-I16](../requirements/ingest.md); knobs: [PLANNER_TUNING.md](../PLANNER_TUNING.md#strategy-selection-tour_planner--fr-i16).

#### Metric-closure on the rep matrix

The connected-component pre-filter guarantees the candidate set is **connected**, not that every *direct* rep pair is routable: two reps in the same `≤ maxLinkMeters` component can still have a `null` direct leg (no OSRM foot route between them, only via intermediates). A tour that happens to place such a pair consecutively then carries a `null` leg → the trim's loop length reads `∞` and it can mildly **over-trim**.

Both planners now compute the **metric closure** of the kept component before any trim runs (`apps/api/src/tours/metric-closure.ts`): an `O(n³)` Floyd–Warshall over the `≤ maxLinkMeters` graph fills each `null` direct leg with the shortest through-component distance, so the solver and the marginal trim always work on a fully-finite matrix. A *finite* direct leg — even one longer than `maxLinkMeters` — is a real OSRM route and is left untouched; the cap only restricts which legs may serve as stepping stones. The greedy path closes the meters matrix (it takes per-leg seconds from the picked OSRM legs); the solver path also closes the seconds matrix, carrying each filled leg's seconds along its shortest-metre path so the two stay coherent. Because the kept set is connected over exactly this graph, every `null` pair in the component gets a finite fill; a pair still unreachable (a one-directional gap) stays `null` and the wire-boundary sanitisation handles it as before.

Paired with this, `ToursService.planLoop` re-validates every result through `PlanResult.parse` before returning. The schema's numeric fields are `.finite()`, so a future non-finite leak surfaces as a **loud server-side 500 at the source** instead of an `Infinity`→JSON `null` that the web's `PlanResult.parse` would drop silently. (Both planners still sanitise non-finite scores at the wire boundary as defence-in-depth.)
