...
| **M4** | Routing infra. Split for shipping: **M4-α** ships `routing` module (OSRM client, `route_legs` cache, getLeg + getMatrix). **M4-β** picks up the deferred BullMQ + Valkey wiring (async `overpass-refresh` queue, serve-stale path). OSRM container preprocesses the chosen OSM extract in `infra/osrm/bootstrap.sh`. | M4-α done · M4-β pending     |
